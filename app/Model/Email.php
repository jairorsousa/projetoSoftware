...
class Email {
	...
	public function Enviar(array $Data) {
		...
		elseif (!Check::Email($this->Data['RemetenteEmail'])):
			$this->Error = ['Erro ao enviar mensagem: O e-mail que você informou não tem um formato válido. Informe seu E-mail!'];
			$this->Result = false;
		else:
			$this->setMail();
			$this->Config();
	...
	private function sendMail() {
		if ($this->Mail->Send()):
			$this->Error = ['Obrigado por entrar em contato: Recebemos sua mensagem e estaremos respondendo em breve!'];
			$this->Result = true;
		else:
			$this->Error = ["Erro ao enviar: Entre em contato com o admin. ( {$this->Mail->ErrorInfo})"];
			$this->Result = false;
		endif;
	}
...
$Dados =[
	...
	"Mensagem" => "Olá Diego, Seu Trabalho foi submetido com sucesso.",
